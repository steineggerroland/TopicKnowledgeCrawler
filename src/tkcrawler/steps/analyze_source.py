...
def analyze_source_item(row: Mapping[str, Any], context: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if source_type in {"rss", "rss+podcast"}:
        out["source_status"] = "ready"
        out.setdefault("configuration_status", None)
    elif source_type == "html":
        out["source_status"] = "ready" if _has_html_configuration(row) else "needs_analysis"
        out["configuration_status"] = row.get("configuration_status") or ("valid" if _has_html_configuration(row) else "missing")
    else:
