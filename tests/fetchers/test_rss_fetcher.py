...
def mock_feed_with_entries():
    ...
    mock_feed.entries = [
        Mock(
            ...
            link="https://example.blogentry/test-1",
...
def mock_feed_with_content_array():
    ...
    mock_entry.link = "https://example.blogentry/test-content-array"
...
def test_fetch_rss_valid_feed(mock_feed_with_entries, mocker):
    ...
    url = "https://mocked-rss.feedurl"
    result = fetch_rss_feed(url)

    assert len(result) == 1
    assert result[0]["title"] == "Test Article 1"
    assert result[0]["link"] == "https://example.blogentry/test-1"
...
def test_fetch_rss_content_array(mock_feed_with_content_array, mocker):
    ...
    assert result[0]["link"] == "https://example.blogentry/test-content-array"
